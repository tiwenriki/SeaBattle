...
namespace SeatBattle.CSharp
{
    public class Board : Control
    {
        ...
        //Draws missed shots around the ship
        private void DrawMissedShotsAroundShip(Ship ship, BoardCellState state)
        {
            var shipRegion = ship.GetShipRegion();
            shipRegion.Inflate1(1, 1);
            var points = shipRegion.GetPoints();
            HumanPlayer humanPlayer = new HumanPlayer();
            foreach (var point in points)
            {
                _cells[point.X, point.Y].State = state;

               // humanPlayer.AddShotResult(point.X, point.Y, ShotResult.Missed);
        ...
        public ShotResult OpenentShotAt(int x, int y)
        {
            ...
            if (ship.IsDrowned)
            {
                ...
                //DrawMissedShotsAroundShip(ship, BoardCellState.MissedShot);
